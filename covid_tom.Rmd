...
```
read data and select desired fields
```{r}
...
data <- data %>% select(date=`Report Date`, district=`VDH Health District`, cases=`Total Cases`, Hospitalizations, Deaths)
...
```{r}
stan <- read_csv("trends/trend_0508.csv")
...
n <- ggplot(fairfax, aes(x=date, y=new_cases)) +
  ...
  labs(title="New Covid-19 Cases",
       subtitle = sprintf("Total Cases: %s", max(fairfax$tot_cases)),
           x=NULL, y=NULL) +
...
d <- ggplot(fairfax, aes(x=date, y=new_dead)) +
  ...
  labs(title="New Covid-19 Deaths",
       subtitle = sprintf("Total Deaths: %s", sum(fairfax$dead)),
               x=NULL, y=NULL) +
